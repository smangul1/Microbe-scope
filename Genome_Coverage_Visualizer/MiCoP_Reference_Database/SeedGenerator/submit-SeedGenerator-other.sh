#!/bin/sh
#$ -S /bin/bash
#$ -N job-Seed-Generator-other-malser
#$ -cwd
#$ -M mealser@gmail.com
#$ -m abe
...
dataDir="/u/scratch2/scratch1/d/dkim/NCBI-RefSeq_filtered/other"
dataDirBasename=`basename $dataDir`		#other			
dataDir="$dataDir/"						

Subdirectory=$(ls $dataDir)
for file in $Subdirectory
do
	if [ -s "/u/scratch2/scratch1/d/dkim/NCBI-RefSeq_filtered_Seeds/${file%.*}.fasta" ] #if file exists, to avoid overwriting existing files 
	then
		echo "$file found."
	else
		...
		then
			...
			python3 /u/project/zarlab/malser/MiCoP/Scripts/NonoverlappingSeedGenerator.py ${dataDir}${file} | awk '!seen[$0]++' | python3 /u/project/zarlab/malser/MiCoP/Scripts/FASTAformatter.py /dev/fd/0 ${file}> "/u/scratch2/scratch1/d/dkim/NCBI-RefSeq_filtered_Seeds/${file%.*}.fasta"
			# to verify that the output of previous command is correct 
			# sort /u/scratch2/scratch2/m/malser/NCBI-RefSeq_filtered_Seeds/other.1.1.fasta | uniq -c |wc -l
		fi
	fi
done
