...
do
	if [ -s "/u/scratch2/scratch1/d/dkim/NCBI-RefSeq_filtered_Seeds/${file%.*}.fastq" ] #if file exists, to avoid overwriting existing files 
	then
		echo "$file found."
	else
		...
		then
			...
			python3 /u/project/zarlab/malser/MiCoP/Scripts/NonoverlappingSeedGenerator.py ${dataDir}${file} | awk '!seen[$0]++' | python3 /u/project/zarlab/malser/MiCoP/Scripts/FASTAformatter.py /dev/fd/0 ${file}> "/u/scratch2/scratch1/d/dkim/NCBI-RefSeq_filtered_Seeds/${file%.*}.fastq"
			# to verify that the output of previous command is correct 
			# sort /u/scratch2/scratch2/m/malser/NCBI-RefSeq_filtered_Seeds/other.1.1.fastq | uniq -c |wc -l
		fi
	fi
done
