# python3 /u/project/zarlab/malser/MiCoP/Scripts/CheckDuplicationFromInputFolder.py /u/scratch2/scratch2/m/malser/NCBI-RefSeq /u/scratch2/scratch2/m/malser/EuPathDB_Organism_List.txt archaea 9
import argparse
import sys
import os
import numpy as np
...
def parseargs():    
	...
	parser.add_argument('THREAD', help='Specific folder in RefSeq. Required.')
...
try:
	for subfile in os.listdir(mypath+str(args.FOLDER)+'/'):
		if subfile.endswith(".genomic.fna.gz") :
			FolderNameLength=len(str(args.FOLDER))
			if (int(subfile[FolderNameLength+1])<=int(args.THREAD)) and (int(subfile[FolderNameLength+1])>(int(args.THREAD)-1)): # this is to parallelize the code on half of the folder				
				try:
				...
				if os.path.exists(str(mypath[:-1])+'_filtered/'+str(args.FOLDER)+'/'+str(subfile[:-3]))==False:
					...
					with gzip.open(str(mypath+str(args.FOLDER)+'/'+subfile), 'rt') as fin:
						for line in fin:
							if line[0]==('>'):
